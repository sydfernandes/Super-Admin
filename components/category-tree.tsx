...
import { Pencil, Trash2, GripVertical, Plus, CirclePlus, ChevronRight } from "lucide-react";
...
/** Add new interface for history log */
...
export function CategoryTree({ className }: CategoryTreeProps) {
  ...
  useEffect(() => {
    async function loadHistory() {
      try {
        const res = await fetch("/api/categories/history");
        const data = await res.json();
      ...
      case "root": {
        if (!tgt.parentId) {
          ...
          if (offsetY < halfHeight) {
            ...
            moveDescription = `Moved "${src.name}" to root level before "${tgt.name}"`;
          } else {
            newFlat.splice(overIndex + 1, 0, { ...src, parentId: null });
            moveDescription = `Moved "${src.name}" to root level after "${tgt.name}"`;
          }
        } else {
  ...
  /** Update addToHistory to persist to API */
  const addToHistory = useCallback(async (action: Omit<CategoryAction, 'id' | 'timestamp'>) => {
    const newAction = {
      ...action,
    ...
    // Persist to API
    try {
      await fetch("/api/categories/history", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  /** Update handlers to include history logging */
              ...
              {items.map((item) => (
                <CategoryTreeItem
                  ...
                  onDelete={async (id: string) => {
                    ...
                    addToHistory({
                      action: 'deleted',
                      details: `Deleted category "${itemToDelete?.name}"`
                    });
                  }}
